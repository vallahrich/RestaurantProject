    ...
    public class ReviewRepository : BaseRepository
    {
        ...
        public Review GetById(int id)
        {
            ...
            try
            {
                ...
                cmd.CommandText = "SELECT * FROM reviews WHERE review_id = @id";
        ...
        public List<Review> GetReviewsByUserId(int userId)
        {
            ...
            try
            {
                dbConn = new NpgsqlConnection(ConnectionString);
                var cmd = dbConn.CreateCommand();
                cmd.CommandText = "SELECT * FROM reviews WHERE user_id = @userId ORDER BY created_at DESC";
        ...
        public double GetAverageRatingForRestaurant(int restaurantId)
        {
            ...
            try
            {
                dbConn = new NpgsqlConnection(ConnectionString);
                var cmd = dbConn.CreateCommand();
                cmd.CommandText = "SELECT AVG(rating) FROM reviews WHERE restaurant_id = @restaurantId";
                cmd.Parameters.Add("@restaurantId", NpgsqlDbType.Integer).Value = restaurantId;
